...
from checks import DMARC_NON_SENDING_POLICY
from checks import DMARC_NON_SENDING_POLICY_ORG
from checks import scoring
from checks import SPF_NON_SENDING_POLICY
...
from checks.tasks.dispatcher import post_callback_hook
...
def do_dkim(self, url, *args, **kwargs):
    try:
        cb_data = self.async_resolv(
            f"_domainkey.{url}", unbound.RR_TYPE_TXT, dkim_callback
        )
        result = dict(
...
def spf_callback(data, status, r):
    ...
    if status == 0:
        ...
        elif r.rcode == unbound.RCODE_NXDOMAIN or (
            r.rcode == unbound.RCODE_NOERROR and r.havedata == 0
        ):
...
def spf_check_include(domain, term, task, policy_records, max_lookups):
    return spf_check_include_redirect(
        ...
        max_lookups,
        ":",
        SpfPolicyStatus.invalid_include,
        is_include=True,
    )


def spf_check_policy(
    domain, spf_record, task, policy_records, max_lookups=10, is_include=False
):
    """
    ...
    if not parsed:
        ...
        for term in (t.lower() for t in parsed["terms"]):
            ...
            if term.startswith("redirect"):
                ...
                # Check 'all'
                if term.startswith(("+", "a")) or (
                    not is_include and term.startswith("?")
                ):
                    status = SpfPolicyStatus.invalid_all
                    score = scoring.MAIL_AUTH_SPF_POLICY_PARTIAL
                break
...
def do_dmarc(self, url, *args, **kwargs):
    try:
        ...
        cb_data = self.async_resolv(
            f"_dmarc.{url}", unbound.RR_TYPE_TXT, dmarc_callback
        )
        if cb_data.get("cont"):
        ...
        if len(record) == 1:
            policy_status, policy_score = dmarc_check_policy(
                record[0], url, self, is_org_domain, public_suffix_list
            )
...
def dmarc_verify_sufficient_policy(parsed, is_org_domain, public_suffix_list):
    ...
    if status == DmarcPolicyStatus.valid:
        if not parsed.get("directives"):
            ...
            if is_org_domain:
                if not parsed["directives"].get("srequest") and not parsed[
                    "directives"
                ].get("request"):
                    status = DmarcPolicyStatus.invalid_p_sp
                    score = scoring.MAIL_AUTH_DMARC_POLICY_PARTIAL
...
def dmarc_get_public_suffix_list():
    ...
    if not is_loading:
        status = cache.add(
            public_suffix_list_loading_id, True, timeout=public_suffix_list_loading_ttl
        )
        if status:
            public_suffix_list = dmarc_fetch_public_suffix_list()
            if public_suffix_list:
                cache.set(
                    public_suffix_list_id,
                    public_suffix_list,
