...
def ipv6_check(
    ...
    for ns_name in ns_names:
        ...
        if r_a and hasattr(r_a, "answer") and r_a.answer:
            try:
                ns_ip4 = [
                    item.address for answer in r_a.answer for item in answer.items
                ][0]
        ...
        if r_aaaa and hasattr(r_aaaa, "answer") and r_aaaa.answer:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"OS error querying IPv6 nameserver {ns_ip6}: {str(e)}"
                    )
                    try:
                        ...
                        if ns_ip4:
                            ...
                            logger.info(
                                f"Name server answer (IPv4 fallback): {tcp6_response.answer}"
                            )
                        else:
                            tcp6_response = None
                    except dns.exception.Timeout:
                        ...
                        logger.warning(
                            f"Error on IPv4 fallback for {ns_name}: {str(e)}"
                        )
                        tcp6_response = None
            except (IndexError, AttributeError) as e:
