...
class RpkiMxHost(RpkiTestHost):
    mailtestrpki = models.ForeignKey(
        MailTestRpki, null=True, related_name="mxhosts", on_delete=models.CASCADE
    )
...
class RpkiNsHost(RpkiTestHost):
    webtestrpki = models.ForeignKey(
        WebTestRpki, null=True, related_name="nshosts", on_delete=models.CASCADE
    )
    mailtestrpki = models.ForeignKey(
        MailTestRpki, null=True, related_name="nshosts", on_delete=models.CASCADE
    )
...
class RpkiMxNsHost(RpkiTestHost):
    mailtestrpki = models.ForeignKey(
        MailTestRpki, null=True, related_name="mxnshosts", on_delete=models.CASCADE
    )
...
class RpkiWebHost(RpkiTestHost):
    webtestrpki = models.ForeignKey(
        WebTestRpki, null=True, related_name="webhosts", on_delete=models.CASCADE
    )
...
class DomainTestReport(models.Model):
    ...
    appsecpriv = models.ForeignKey(
        WebTestAppsecpriv, null=True, on_delete=models.CASCADE
    )
...
class BatchRequest(models.Model):
    ...
    user = models.ForeignKey(
        BatchUser, related_name="batch_requests", on_delete=models.CASCADE
    )
    name = models.CharField(max_length=255)
    ...
    status = EnumIntegerField(
        BatchRequestStatus, default=BatchRequestStatus.registering, db_index=True
    )
    ...
    def delete_related_data(self, delete_self=False):
        ...
        batch_webtest_ids = {
            d.webtest_id for d in BatchDomain.objects.filter(batch_request=self).all()
        }
        batch_mailtest_ids = {
            d.mailtest_id for d in BatchDomain.objects.filter(batch_request=self).all()
        }
...
class BatchDomain(models.Model):
    ...
    batch_request = models.ForeignKey(
        BatchRequest, related_name="domains", on_delete=models.CASCADE
    )
    status = EnumIntegerField(
