...
class SetupUnboundContext(Task):
    ...
    def ub_ctx(self):
        if self._ub_ctx is None:
            ...
            if (
                hasattr(settings, "ENABLE_INTEGRATION_TEST")
    ...
    def resolve(self, qname, qtype):
        ...
        if "data" in resp:
            if qtype == unbound.RR_TYPE_AAAA:
                return [
                    socket.inet_ntop(socket.AF_INET6, rr) for rr in resp["data"].data
                ]
            elif qtype == unbound.RR_TYPE_A:
                return [
                    socket.inet_ntop(socket.AF_INET, rr) for rr in resp["data"].data
                ]
